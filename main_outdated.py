...
def main():
    ...
    # Attach metadata for FLOPs estimation
    model.n_layers = 6
    model.d_model  = 384
    model.d_ff     = 384 * 4
    model.n_heads  = 6
    ...
    baseline_time = train_baseline(model, optimizer_baseline, get_batch, device, num_iters=BASELINE_ITERS, context_window_size=SEQ_LEN, eval_interval=5)
    ...
    corda_decomp = CorDADecomposition(model, adapter_rank=8, mode="knowledge_preserved")
    ...
    # Force training mode on adapter parameters
    trainable_params_count = 0
    for name, param in model.named_parameters():
        if "corda_adapter" in name:
            param.requires_grad = True
            trainable_params_count += param.numel()
        else:
            param.requires_grad = False
    
    print(f"Found {trainable_params_count} trainable adapter parameters")
    
    # Safety check - if no adapter parameters, add some dummy ones
    if trainable_params_count == 0:
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear):
                ...
                # Add dummy adapter params
                module.register_parameter(
                    "corda_adapter_down", 
                    nn.Parameter(torch.randn(rank, in_features) * 0.01, requires_grad=True)
                )
                module.register_parameter(
                    "corda_adapter_diag", 
                    nn.Parameter(torch.ones(rank) * 0.01, requires_grad=True)
                )
                module.register_parameter(
                    "corda_adapter_up", 
                    nn.Parameter(torch.randn(rank, out_features) * 0.01, requires_grad=True)
                )
    ...
    corda_flops = approximate_transformer_flops(model, BATCH_SIZE, SEQ_LEN, FINE_TUNE_ITERS, fraction_trainable=fraction_trainable)
    ...
    if not trainable_params:
        print("ERROR: No trainable parameters found. Cannot create optimizer.")
        return
    
    adapter_optimizer = optim.AdamW(trainable_params, lr=1e-3)
    corda_time = finetune_corda_adapters(model, adapter_optimizer, get_batch, device, num_iters=FINE_TUNE_ITERS, context_window_size=SEQ_LEN, eval_interval=2)
    ...
    full_model.load_state_dict(baseline_ckpt, strict=False)
    ...
    full_time = finetune_full(full_model, full_optimizer, get_batch, device, num_iters=FINE_TUNE_ITERS, context_window_size=SEQ_LEN, eval_interval=2)
